...
class WazeRouteCalculator(object):
    ...
    def __init__(self, start_address, end_address, log_lvl=logging.INFO):
        ...
        #print ('Start coords: (%s, %s)', self.start_coords["lon"], self.start_coords["lat"])
        self.end_coords = self.address_to_coords(end_address)
        self.log.debug('End coords: (%s, %s)', self.end_coords["lon"], self.end_coords["lat"])
        #print ('End coords: (%s, %s)', self.start_coords["lon"], self.start_coords["lat"])
    ...
    def get_route(self):
        ...
        response = urllib.urlopen("https://www.waze.com/row-RoutingManager/routingRequest?from=x%3A-79.655+y%3A8.956&to=x%3A-79.563+y%3A8.943&at=1&returnJSON=true&returnGeometries=false&returnInstructions=true&timeout=60000&nPaths=1&options=AVOID_TRAILS").read()
...
class WazeRouteNotification():
    ...
    def get_notification(self, ticks):
        """Get route data from waze"""		
        Url = "https://www.waze.com/row-rtserver/web/TGeoRSS?ma=400&mj=100&mu=100&left=-79.657&right=-79.563&bottom=8.940&top=8.959&_=" + str(int(ticks))
        #print Url 
        rnotification = urllib.urlopen(Url).read()
        #rnotification = urllib.urlopen("https://www.waze.com/row-rtserver/web/TGeoRSS?ma=400&mj=100&mu=100&left=-79.657&right=-79.563&bottom=8.940&top=8.959&_=1480287289615").read()
        #rnotification = urllib.urlopen("https://www.waze.com/row-rtserver/web/TGeoRSS?ma=400&mj=100&mu=100&left=-79.657&right=-79.563&bottom=8.940&top=8.959&_=1478382671317").read()
        #response = urllib.urlopen("http://localhost:8080/waze/routesWithDirections?start=Interamericana%2C%20Arraijan%2C%20Panama&end=Puente%20De%20Las%20Americas%2C%20%20Panama").read()
        #response = urllib.urlopen(self.LOCAL_URL+"?latTop=8.959&lonLeft=-79.655&latBottom=8.940&lonRight=-79.563").read()
        rnotification_json = json.loads(rnotification)
        
        #print json.dumps(rnotification_json)
        #print rnotification_json
